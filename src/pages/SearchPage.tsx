...
import { ArrowLeft, Filter, MapPin, Car } from 'lucide-react';
...
export function SearchPage({ navigate }: SearchPageProps) {
  return (
    <div className="min-h-screen bg-background flex flex-col pb-20">
      <div className="bg-card px-4 pt-12 pb-4 shadow-sm z-10">
        ...
        <div className="flex items-center justify-between bg-muted p-3 rounded-lg text-sm">
          ...
          <span className="text-muted-foreground">Auj.</span>
        </div>
      </div>

      <RFTabs defaultValue="list" className="flex-1 flex flex-col w-full">
        <div className="px-4 py-3 bg-card border-b border-border">
          <RFTabsList className="w-full grid grid-cols-2">
            <RFTabsTrigger value="list">Liste</RFTabsTrigger>
            <RFTabsTrigger value="map">Carte</RFTabsTrigger>
          </RFTabsList>
        </div>

        <RFTabsContent value="list" className="flex-1 p-4 space-y-4 m-0">
          {trips.map((trip) => (
              ...
              <RFCardContent className="p-4">
                ...
                {trip.type === 'available' && (
                  <div className="mt-4 pt-3 border-t border-border">
                    <RFBadge variant="secondary" className="bg-secondary/10 text-brand-teal">Chauffeur disponible maintenant</RFBadge>
                  </div>
                )}
              </RFCardContent>
            </RFCard>
        ...
        <RFTabsContent value="map" className="flex-1 m-0 relative">
          <div className="absolute inset-0 bg-primary/5 flex items-center justify-center">
            <div className="text-center">
              <MapPin className="w-12 h-12 text-brand-blue mx-auto mb-2 opacity-50" />
              <p className="text-muted-foreground font-medium">Carte interactive</p>
              <p className="text-sm text-muted-foreground">Affichage des trajets et chauffeurs</p>
            </div>
            <div className="absolute top-1/4 left-1/4 w-8 h-8 bg-card rounded-full shadow-lg flex items-center justify-center border-2 border-brand-blue">
              <Car className="w-4 h-4 text-brand-blue" />
            </div>
            <div className="absolute top-1/2 right-1/3 w-8 h-8 bg-card rounded-full shadow-lg flex items-center justify-center border-2 border-brand-teal">
