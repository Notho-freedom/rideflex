...
import { MapPin, Calendar, Car, Bell, UserCircle, Radio, ClipboardList, MoreHorizontal, HandHelping, Search, Loader2 } from 'lucide-react';
...
export function HomePage({ navigate }: HomePageProps) {
  ...
  return (
    <div className="min-h-screen bg-background pb-20 lg:pb-8">
        ...
        <div className="max-w-4xl mx-auto relative z-10">
          <div className="flex justify-between items-center mb-6">
            <div>
              ...
              <span className="inline-block mt-1 px-2.5 py-0.5 rounded-full text-xs font-semibold bg-white/20 text-primary-foreground">
            ...
            <div className="flex space-x-3">
              <button onClick={() => navigate('notifications')} className="p-2 bg-white/20 rounded-full text-primary-foreground hover:bg-white/30 transition relative">
                ...
                {unreadCount > 0 && <span className="absolute top-0 right-0 w-2.5 h-2.5 bg-red-500 rounded-full border-2 border-transparent" />}
              </button>
          ...
          <p className="text-primary-foreground/90 text-lg font-medium">{isDriver ? 'Rentabilisez vos trajets.' : 'Le covoiturage réinventé.'}</p>
          <p className="text-primary-foreground/80 text-sm mt-1">{isDriver ? 'Publiez et trouvez des passagers.' : 'Trouvez votre trajet idéal aujourd\'hui.'}</p>
        </div>
      </div>
      ...
      <div className="px-4 mt-8 max-w-2xl lg:max-w-4xl mx-auto">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          {quickActions.map((action, i) => (
            <RFCard key={i} className={`cursor-pointer transition-colors ${action.color === 'secondary' ? 'hover:border-secondary' : 'hover:border-primary'}`} onClick={() => navigate(action.page)}>
              <RFCardContent className="p-4 flex flex-col items-center text-center space-y-2">
                <div className={`w-12 h-12 rounded-full flex items-center justify-center ${action.color === 'secondary' ? 'bg-secondary/10 text-secondary' : 'bg-primary/10 text-primary'}`}>
      ...
      <div className="px-4 mt-8 max-w-2xl lg:max-w-4xl mx-auto">
        <h2 className="text-lg font-bold text-foreground mb-4">{isDriver ? 'Dernières demandes' : 'Trajets récents'}</h2>
        {loadingTrips ? (
          <div className="flex justify-center py-8"><Loader2 className="w-6 h-6 animate-spin text-primary" /></div>
        ) : recentTrips.length === 0 ? (
          <div className="text-center py-8 text-muted-foreground"><p>Aucun trajet récent.</p></div>
        ) : (
              ...
              <RFCard key={trip.id} className="cursor-pointer" onClick={() => navigate('trip-detail', { tripId: trip.id })}>
                <RFCardContent className="p-4">
                  ...
                  <div className="flex items-center text-sm text-muted-foreground space-x-2">
                    <span className="font-medium text-foreground">{trip.from_city}</span>
                    <span>→</span>
