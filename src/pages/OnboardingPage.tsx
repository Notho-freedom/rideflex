...
const slides = [
  {
    ...
    desc: 'Recherchez parmi des milliers de trajets proposés par des chauffeurs vérifiés, partout dans le monde.',
    illustration: (
    ...
    <div className="min-h-screen bg-card flex flex-col lg:flex-row">
      ...
      <div className="flex-1 flex flex-col items-center justify-center px-6 py-12">
        <div className="max-w-sm w-full text-center">
          <div className="flex justify-center mb-8">{slide.illustration}</div>
          ...
          <div className="flex justify-center space-x-2 mb-8">
            {slides.map((_, i) => (
              <button key={i} onClick={() => setCurrent(i)} className={`h-2 rounded-full transition-all ${i === current ? 'w-8 bg-gradient-brand' : 'w-2 bg-muted-foreground/30'}`} />
            ))}
          </div>
          <div className="flex items-center justify-between gap-4">
            {current > 0 ? (
              ...
              <button onClick={() => navigate('auth')} className="text-sm text-muted-foreground hover:text-foreground transition-colors">Passer</button>
            )}
            {isLast ? (
              <RFButton variant="brand" size="xl" onClick={() => navigate('auth')} className="flex-1">Commencer</RFButton>
            ) : (
              <RFButton variant="brand" onClick={() => setCurrent(current + 1)} className="flex-1">Suivant<ChevronRight className="w-4 h-4 ml-1" /></RFButton>
            )}
          </div>
        </div>
      </div>
